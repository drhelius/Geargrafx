---
name: Issue Triage
description: Triage newly opened Geargrafx issues by applying the correct repository labels.
on:
  issues:
    types: [opened]
    lock-for-agent: true
...
4. Post a clarification comment only when the issue is likely a `bug`, the next engineering action depends on Geargrafx version or platform/frontend, and the issue body does not already provide that information.
5. When asking, ask only for Geargrafx version and platform/frontend. Do not ask for ROM hashes, logs, screenshots, save files, long reproduction forms, or extra context during initial triage.
...
- `bug`: broken behavior in an already-supported Geargrafx feature, mapper, platform, build, package, or workflow. Use this for regressions, crashes, incorrect emulation, save-state problems, audio/video/input defects, frontend bugs, and release or CI breakages caused by this repository.
...
- `question`: a user question, support request, usage question, or troubleshooting question where the user is asking how something works or how to use Geargrafx.
- `not a bug`: expected behavior, invalid or corrupted ROM/save data, a known hardware limitation, an external service problem, spam/test content, or behavior caused by user environment rather than Geargrafx.
...
### Phase 4: Geargrafx-Specific Checks

For game compatibility reports, separate bugs from unsupported cartridge hardware:

1. Treat likely unsupported HuCard banking, CD-ROM/System Card/Arcade Card, SuperGrafx, or expansion hardware as `feature request`. Clues include unsupported system-card requirements, special CD hardware behavior, unusual bank switching, or a game that fails only because Geargrafx has no implementation for that hardware type.
...
3. Treat bad dumps, patched/trainer ROM issues without clean-ROM evidence, invalid save files, or expected TurboGrafx-16 / PC Engine hardware behavior as `not a bug` when the evidence is clear.
4. Use `needs info` when the report says only that a game does not run but omits the exact ROM title/region/revision, ROM hash, Geargrafx version, platform, steps, or whether a clean dump was tested.
...
3. For clarification comments, use a clearly automated but readable format, for example: `Automated triage request: please provide the Geargrafx version and platform/frontend so this bug report can be classified correctly.`
