...
TraceLogger::TraceLogger(const u64* master_clock_cycles)
{
    ...
    m_buffer = new (std::nothrow) GG_Trace_Entry[TRACE_BUFFER_SIZE];
#else
    m_buffer = NULL;
#endif
...
bool TraceLogger::SetCapacity(u32 capacity)
{
    ...
    GG_Trace_Entry* buffer = new (std::nothrow) GG_Trace_Entry[capacity];
...
void TraceLogger::SetEventFilter(GG_Trace_Type type, u32 filter)
{
...
u32 TraceLogger::GetEventFilter(GG_Trace_Type type) const
{
    if (type < TRACE_TYPE_COUNT)
...
const GG_Trace_Entry& TraceLogger::GetEntry(u32 index) const
{
    static const GG_Trace_Entry k_empty = {};
