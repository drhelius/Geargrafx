...
final class EmulationViewController: UIViewController, UIAdaptivePresentationControllerDelegate {
    private var rom: Rom
    private let emulator = GeargrafxEmulator()
    private let backgroundView = GameplayBackgroundView(frame: .zero)
    private let screenView = GameScreenView(frame: .zero)
    private let controlsView = GameControlsView(controllerType: AppSettings.controller)
    ...
    private var nativeScreenHeight: CGFloat = 224.0
    ...
    private func configureViews() {
        ...
        emulator.configure(
            ...
            palette: AppSettings.palette.rawValue,
            overscan: AppSettings.overscanEnabled,
            scanlineMode: AppSettings.scanlineMode.rawValue,
            scanlineStart: AppSettings.scanlineStart,
            scanlineEnd: AppSettings.scanlineEnd,
            noSpriteLimit: AppSettings.noSpriteLimitEnabled,
            safeVDCDefaults: AppSettings.safeVDCDefaultsEnabled,
            lowpassFilter: AppSettings.lowpassFilterEnabled,
            lowpassIntensity: AppSettings.lowpassIntensity,
            lowpassCutoff: AppSettings.lowpassCutoff.rawValue,
            controllerType: AppSettings.controller.rawValue,
            avenuePad3MainButton: AppSettings.avenuePad3MainButton.rawValue,
            softResetEnabled: AppSettings.softResetEnabled,
            turboIEnabled: AppSettings.turboIEnabled,
            ...
            cdromType: AppSettings.cdromType.rawValue,
            cdromBIOS: AppSettings.cdromBIOS.rawValue,
            preloadCDROM: AppSettings.preloadCDROMEnabled,
    ...
    private func layoutScreen() {
        ...
        if landscape {
            ...
            let right = controlsView.actionContainer.frame.minX - 12.0
    ...
    private func updateScreenGeometry() {
        ...
        let widthScale = max(emulator.frameWidthScale, 1)
        let nativeWidth = CGFloat(width) / CGFloat(widthScale)
        let nativeHeight = CGFloat(height)
        let aspectRatio = nativeWidth / nativeHeight
        guard abs(nativeWidth - nativeScreenWidth) > 0.0001 ||
              abs(nativeHeight - nativeScreenHeight) > 0.0001 ||
              abs(aspectRatio - screenAspectRatio) > 0.0001 else { return }

        screenAspectRatio = aspectRatio
        nativeScreenWidth = nativeWidth
        nativeScreenHeight = nativeHeight
        view.setNeedsLayout()
    }

    private func emulatorButton(for button: GameControllerInput.Button) -> GeargrafxButton? {
        switch button {
        case .up: return .up
        ...
        case .x: return .III
        case .y: return .IV
        case .leftShoulder: return .V
        case .rightShoulder: return .VI
        case .menu: return .run
