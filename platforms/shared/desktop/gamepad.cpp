...
static bool gamepad_shortcut_prev[GG_MAX_GAMEPADS][config_HotkeyIndex_COUNT] = { };
...
void gamepad_destroy(void)
{
    for (int i = 0; i < GG_MAX_GAMEPADS; i++)
    {
        ...
        {
            SDL_GameControllerClose(gamepad_controller[i]);
        }
    }
}
...
void gamepad_add(void)
{
    for (int i = 0; i < GG_MAX_GAMEPADS; i++)
    {
        if (IsValidPointer(gamepad_controller[i]))
        {
            SDL_Joystick* js = SDL_GameControllerGetJoystick(gamepad_controller[i]);
    ...
    for (int i = 0; i < SDL_NumJoysticks(); i++)
    {
        ...
        for (int p = 0; p < GG_MAX_GAMEPADS; p++)
        {
            ...
            {
                SDL_JoystickID player_id = SDL_JoystickInstanceID(SDL_GameControllerGetJoystick(gamepad_controller[p]));
...
void gamepad_remove(SDL_JoystickID instance_id)
{
    for (int i = 0; i < GG_MAX_GAMEPADS; i++)
    {
        ...
        {
            SDL_JoystickID current_id = SDL_JoystickInstanceID(SDL_GameControllerGetJoystick(gamepad_controller[i]));
...
void gamepad_assign(int slot, int device_index)
{
    ...
    if (device_index < 0)
    {
        ...
        {
            SDL_GameControllerClose(gamepad_controller[slot]);
    ...
    if (IsValidPointer(gamepad_controller[slot]))
    {
        SDL_JoystickID current_id = SDL_JoystickInstanceID(SDL_GameControllerGetJoystick(gamepad_controller[slot]));
    ...
    for (int i = 0; i < GG_MAX_GAMEPADS; i++)
    {
        ...
        if (IsValidPointer(gamepad_controller[i]))
        {
            SDL_JoystickID id = SDL_JoystickInstanceID(SDL_GameControllerGetJoystick(gamepad_controller[i]));
            if (id == wanted_id)
            {
                other = i;
                break;
...
bool gamepad_get_button(SDL_GameController* controller, int mapping)
{
    if (!IsValidPointer(controller))
        return false;
