...
int application_headless_init(const char* rom_file, const char* symbol_file, int mcp_mode, int mcp_tcp_port)
{
    ...
    if (!config_emulator.syscard_bios_path.empty())
    {
        Log("Loading System Card BIOS: %s", config_emulator.syscard_bios_path.c_str());
        emu_load_bios(config_emulator.syscard_bios_path.c_str(), true);
    }

    if (!config_emulator.gameexpress_bios_path.empty())
    {
        Log("Loading Game Express BIOS: %s", config_emulator.gameexpress_bios_path.c_str());
