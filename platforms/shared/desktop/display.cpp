...
static bool last_turbolink_active = false;
...
void display_use_vsync_if_enabled(void)
{
    ...
    bool effective = config_video.sync_mode != config_VideoSync_Disabled &&
        !display_is_vsync_forced_off() && !emu_turbolink_is_active();
...
void display_update_vsync_state(void)
{
    ...
    if (active == last_turbolink_active)
        return;

    last_turbolink_active = active;
    display_use_vsync_if_enabled();
}
...
static bool display_update_content_frame_rate(void)
{
    ...
    // The VCE can alternate 262 and 263-line frames, so preserve the elapsed phase.
